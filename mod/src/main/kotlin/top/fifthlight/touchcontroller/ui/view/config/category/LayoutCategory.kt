...
import top.fifthlight.combine.platform.MinecraftCanvas
...
private fun BoxScope.ControllerWidget(config: ControllerWidget) {
    ...
    val context = Context(
        ...
        condition = LayoutLayerCondition(),
    )
    config.layout(context)
    Canvas(modifier) {
        val canvas = (canvas as MinecraftCanvas)
        drawQueue.execute(canvas.drawContext, canvas.textRenderer)
    }
}
